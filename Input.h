...
public:
	...
	static void onButtonCallback(int button, int action, int mods);
	...
	static vec2f getMousePos() { return sMouse; }
	static vec2f getMouseOffset() { return sLastMouse - sMouse; }
	static vec2f getMouseScroll() { return sScroll; }
	static void setMousePos(float x, float y) { sMouse = { x, y }; }
	static void setMouseScroll(float x, float y) { sScroll = { x, y }; }


private:
	static bool sKeys[ MAX_KEYS ];
	static bool sLastKeys[ MAX_KEYS ];

	static bool sButtons[ MAX_BUTTONS ];
	static bool sLastButtons[ MAX_BUTTONS ];

	static vec2f sMouse;
	static vec2f sLastMouse;
	static vec2f sScroll;
};
